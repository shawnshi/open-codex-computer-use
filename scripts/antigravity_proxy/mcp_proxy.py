...
log_file = Path(os.environ.get("USERPROFILE", "C:\\")) / ".gemini" / "mcp_proxy.log"
...
def should_block_intrusive_action(app_target_name: str) -> bool:
    ...
    if app_target_name and app_target_name.lower() in fg_title.lower():
        return False
    logging.warning(f"Focus Guard Triggered! User active in '{fg_title}'. Idle time: {idle_ms}ms. Blocking action on '{app_target_name}'.")
    return True
...
def filter_uia_tree(tree_text: str) -> str:
    ...
    for line in lines:
        ...
        if "window: " in lower_line:
            try: target_window_title = line.split('"')[1]
            except: pass
...
def smart_locator_fallback(original_index: int, target_name: str) -> int:
    ...
    for line in last_known_tree_lines:
        ...
        if line_stripped.startswith(f"{original_index} "):
            if target_name.lower() in line_stripped.lower(): return original_index
            else: break
    ...
    for line in last_known_tree_lines:
        ...
        if target_name.lower() in line_stripped.lower():
            try:
                ...
                if dist < min_dist: min_dist = dist; best_match_index = idx
            except: pass
            ...
            if msg_str.startswith("{"):
                ...
                if "method" in msg and msg["method"] == "tools/call":
                    ...
                    if tool_name in ["click", "type_text", "press_key", "set_value"]:
                        ...
                        if should_block_intrusive_action(app_name):
                            reject_msg = {
                                ...
                                "result": {"isError": True, "content": [{"type": "text", "text": "Focus Guard Blocked: User is actively typing or using the mouse. Please back off."}]}
                            }
                    ...
                    if tool_name in ["click", "set_value"] and "element_index" in tool_args:
                        ...
                        if target_name: tool_args["element_index"] = smart_locator_fallback(tool_args["element_index"], target_name)
                            
                    if tool_name in ["click", "drag"]: scale_coordinates(tool_args)
